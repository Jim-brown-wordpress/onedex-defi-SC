    ...
    + crate::event::EventModule
{
    ...
    fn add_initial_liquidity(&self) {
        ...
        // initial total liquidity depends on wegld amount
        // it wegld does not exist on pair, it depends on usdc amount
        // convert decimal as 18
        let initial_total_liquidity = 
            if first_payment.token_identifier == self.wegld_token_id().get() ||
                first_payment.token_identifier == self.busd_token_id().get()
            {
                first_payment.amount.clone()
            } else if second_payment.token_identifier == self.wegld_token_id().get() ||
                second_payment.token_identifier == self.busd_token_id().get()
            {
                second_payment.amount.clone()
            } else if 
                first_payment.token_identifier == self.usdc_token_id().get() ||
                first_payment.token_identifier == self.usdt_token_id().get()
            {
                first_payment.amount.clone() * BigUint::from(1_000_000_000_000u64)
            } else if 
                second_payment.token_identifier == self.usdc_token_id().get() ||
                second_payment.token_identifier == self.usdt_token_id().get()
            {
                second_payment.amount.clone() * BigUint::from(1_000_000_000_000u64)
            } else {
                sc_panic!("invalid pair tokens");
            };
        ...
        self.add_initial_liquidity_event(
            self.blockchain().get_caller(),
    ...
    fn add_liquidity(
        ...
        let (first_token_amount_added, second_token_amount_added) = if second_token_amount_optimal <= second_payment.amount {
            require!(
                ...
                "insufficient frist token amount"
            );
            ...
            require!(
                second_payment.amount.clone() >= second_token_amount_min,
                "insufficient second token amount"
            );
    ...
    fn remove_liquidity(
        ...
        if self.pair_first_token_id(pair_id).get() == self.wegld_token_id().get() && unwrap_required {
        ...
        if self.pair_second_token_id(pair_id).get() == self.wegld_token_id().get() && unwrap_required {
            let mut unwrap_payment = ManagedVec::new();
            unwrap_payment.push(
                EsdtTokenPayment::new(
                    self.wegld_token_id().get(), 
